{{-- ui/status-check-row — icon + wrapping label, lineHeight 1.4.
     ...
     status the source never showed either — inferred the same way: the amber pair the translation-
     status "draft" chip already uses (inspector.blade.php), paired with Phosphor's warning-circle
     (only vendored in the "regular"/hollow weight — EditorIcon falls back to it automatically, same
     as any other icon this package requests in a weight that isn't vendored). --}}
@once
<style>
    ...
    .hb-statuscheckrow__icon--warn { color: #8A5A00; }
