...
@props(['mode' => 'color', 'selected' => 'border', 'tokens' => null])
@php
    $default = $mode === 'number'
...
<div class="hb-pop hb-varmenu" data-hb-varmenu>
    ...
    <div class="hb-varmenu__list">
        @foreach ($items as $name => $val)
            <button type="button" data-vm-name="{{ $name }}" class="hb-vmi{{ $name === $selected ? ' hb-vmi--on' : '' }}">
    ...
    (() => {
        const boot = () => document.querySelectorAll('[data-hb-varmenu]').forEach((menu) => {
            ...
            items.forEach((it) => it.addEventListener('click', () => {
                ...
                menu.dispatchEvent(new CustomEvent('varselect', { bubbles: true, detail: { name: it.dataset.vmName } }));
            }));
