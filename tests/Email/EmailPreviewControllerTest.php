...
class EmailPreviewControllerTest extends TestCase
{
    ...
    // and export endpoints. Heisenberg never substitutes values; the host's
    // newsletter integration reads the rendered text and substitutes at
    // send time. The raw `{{ ... }}` token must therefore reach every
    // author-facing GET, untouched.
    // ====================================================================

    private function makeEmailWithVariableToken(string $subject = 'Hello {{ user.first_name }}'): Post
    {
        $post = Post::create([
            'title_en' => $subject,
            'locale' => 'en',
    ...
    public function test_show_by_slug_passes_placeholders_through_verbatim(): void
    {
    ...
    public function test_size_endpoint_responds_200_for_an_email_with_placeholders(): void
    {
        $post = $this->makeEmailWithVariableToken();
    ...
    public function test_id_scoped_preview_redirects_to_the_slug_url(): void
    {
        $post = $this->makeEmailWithVariableToken();

        $this->get("/editor/{$post->id}/email-preview")
            ->assertRedirect("/emails/{$post->slug}");
    }
}
