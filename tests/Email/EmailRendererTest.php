...
class EmailRendererTest extends TestCase
{
    ...
    // substitute values; the host's newsletter integration reads the
    // rendered text and substitutes at send time. Both the html and text
    // outputs must carry the literal token, the stored Block payload must
    // still carry the token after the render, and re-rendering must
    // produce byte-identical output.
    // ====================================================================

    public function test_variable_placeholders_are_passed_through_verbatim(): void
    {
        $post = $this->variableTokenFixture();

        $result = $this->renderer()->render($post, 'en');

        // The literal token is in the subject, every text-bearing block,
        // and the plain-text alternative — exactly what the author typed.
        $this->assertSame('Welcome {{ user.first_name }}', $result->subject);
        $this->assertStringContainsString('Hi {{ user.first_name }}', $result->html);
        $this->assertStringContainsString('Hello {{ user.first_name }}', $result->html);
        $this->assertStringContainsString('Hi {{ user.first_name }}', $result->text);
        $this->assertStringContainsString('Hello {{ user.first_name }}', $result->text);

        // The persisted post + block payload is untouched by rendering —
        // no implicit substitution has written back to storage.
        $post->refresh();
        $this->assertSame('Welcome {{ user.first_name }}', $post->title_en);
        foreach ($post->blocks as $block) {
            $serialized = (string) json_encode($block->content);
            $this->assertStringContainsString('{{ user.first_name }}', $serialized);
        }

        // Re-rendering is idempotent — the same output, byte for byte.
        $again = $this->renderer()->render($post, 'en');
        $this->assertSame($result->html, $again->html);
        $this->assertSame($result->text, $again->text);
        $this->assertSame($result->subject, $again->subject);
    }

    private function variableTokenFixture(): Post
    {
        $post = Post::create([
