...
class EmailExportControllerTest extends TestCase
{
    ...
    // export endpoints. Heisenberg never substitutes values; the host's
    // newsletter integration reads the rendered text and substitutes at
    // send time. The raw `{{ ... }}` token must therefore reach every
    // export, untouched.
    // ====================================================================

    private function makeEmailWithVariableTokens(string $slug = 'token-newsletter'): Post
    {
        $post = Post::create([
    ...
    public function test_html_export_passes_placeholders_through_verbatim(): void
    {
        $post = $this->makeEmailWithVariableTokens();

        $response = $this->get("/emails/{$post->slug}/export?format=html");

        $response->assertOk();
        $html = (string) $response->getContent();
        $this->assertStringContainsString('Hi {{ user.first_name }}', $html);
        $this->assertStringContainsString('Read more from {{ user.first_name }}', $html);
        $this->assertStringContainsString('{{ unsubscribe_url }}', $html);
        $this->assertStringNotContainsString('Bonjour', $html, 'en locale should be used');
    }

    public function test_html_export_uses_en_locale_by_default_and_fr_when_asked(): void
    {
        $post = $this->makeEmailWithVariableTokens();

        $en = (string) $this->get("/emails/{$post->slug}/export?format=html")->assertOk()->getContent();
        $fr = (string) $this->get("/emails/{$post->slug}/export?format=html&locale=fr")->assertOk()->getContent();

        $this->assertStringContainsString('Hello {{ user.first_name }}', $en);
        $this->assertStringContainsString('Bonjour {{ user.first_name }}', $fr);
    }

    public function test_eml_export_passes_placeholders_through_verbatim(): void
    {
        config(['mail.from.address' => 'sender@example.test', 'mail.from.name' => 'Example Sender']);
        $post = $this->makeEmailWithVariableTokens();
...
"], '', $raw);
        ...
        $this->assertStringContainsString('{{ unsubscribe_url }}', $dequoted);
    }
}
