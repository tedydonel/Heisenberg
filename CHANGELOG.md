...
- **The topbar's home button is configured on its own, per role.** It is navigation — where the person editing came from — not identity, which is what `site_url` answers, so a platform whose admins and staff have different dashboards can return each of them to their own. `heisenberg.dashboard_url` takes a single URL, or a map keyed by your own role names with an optional `default`; unset, it falls back to `site_url` exactly as before. SEO still answers from `site_url` alone.
...
- **Three theme presets ship instead of six** (Default, Midnight, Sunset). A preset only seeds the colour swatches; saving under your own name now also deselects it, so an author's theme is visibly their own rather than looking like the preset it started from. A saved theme was already a complete, independent copy of every token.
