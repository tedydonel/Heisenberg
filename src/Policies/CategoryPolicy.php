 ...
 * update/delete" = super_admin/admin only — "identical" to BlogTagPolicy,
 * see TagPolicy).
 *
 * Heisenberg's `authors` tier (config('heisenberg.roles')) already IS "any
 * content role" — it's a superset containing super_admin, admin, and the
 * three employee levels — so `is($user, 'authors')` alone covers the whole
 * first row; `admins` (super_admin/admin) covers the second. Same
 * LocalDevRoleGate wrapping as PostPolicy, for the identical reason: these
 * routes sit behind config('heisenberg.middleware.editor') (unauthenticated
 * by default), so a GuestActor stand-in needs the same environment-gated
 * local-dev bypass PostPolicy's docblock documents in full.
 */
